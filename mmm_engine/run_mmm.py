...
def run(excel_path: str,
    ...
    summary = {
        ...
        'actual_vs_pred': {
            'dates_train':  [str(d)[:10] for d in final_metrics['dates_train']],
            'actual_train': [float(x) for x in final_metrics['actual_train_cv']],
            'pred_train':   [float(x) for x in (final_metrics['pred_train_sqrt'] ** 2)],
            'dates_hold':   [str(d)[:10] for d in final_metrics['dates_hold']],
            'actual_hold':  [float(x) for x in final_metrics['actual_hold_cv']],
            'pred_hold':    [float(x) for x in (final_metrics['pred_hold_sqrt'] ** 2)],
        },
        'r2': final_metrics['r2'],
